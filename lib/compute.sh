...
install_compute_packages() {
    ...
    # Install GPU plugins if GPUs were detected
    # These packages are in 'contrib' and may not be available on all systems
    if [[ "${LOCAL_HAS_NVIDIA:-false}" == "true" ]]; then
        log_info "NVIDIA GPU detected — installing NVML plugin for GPU scheduling..."
        if ! apt_install slurm-wlm-nvml-plugin 2>/dev/null; then
            log_warn "Could not install slurm-wlm-nvml-plugin."
            log_warn "GPU auto-detection requires this package from the 'contrib' repository."
            log_warn "Enable contrib and run: sudo apt install slurm-wlm-nvml-plugin"
        fi
    fi

    if [[ "${LOCAL_HAS_AMD:-false}" == "true" ]]; then
        log_info "AMD GPU detected — installing RSMI plugin for GPU scheduling..."
        if ! apt_install slurm-wlm-rsmi-plugin 2>/dev/null; then
            log_warn "Could not install slurm-wlm-rsmi-plugin."
            log_warn "GPU auto-detection requires this package from the 'contrib' repository."
            log_warn "Enable contrib and run: sudo apt install slurm-wlm-rsmi-plugin"
        fi
    fi
...
# Only includes AutoDetect lines for plugins that are actually available.
setup_gres_conf_compute() {
    ...
    if ! $has_nvml && ! $has_rsmi; then
        ...
        log_info "Install slurm-wlm-nvml-plugin (NVIDIA) or slurm-wlm-rsmi-plugin (AMD) for GPU support."
        return 0
    fi
